...
const appCss = "/assets/styles-BGmWXD47.css";
const store = /* @__PURE__ */ new Map();
const listeners = /* @__PURE__ */ new Map();
const STORAGE_KEY = "buurtmatch-demo-store-v1";
const EMPTY_ROWS = [];
...
function hydrateFromStorage() {
  ...
  try {
    ...
    Object.entries(parsed).forEach(([table, rows]) => {
      if (Array.isArray(rows)) store.set(table, rows);
    });
  } catch (error) {
    console.warn("Failed to hydrate demo store", error);
  }
}
function persistToStorage() {
  if (typeof window === "undefined") return;
  try {
    window.localStorage.setItem(STORAGE_KEY, JSON.stringify(Object.fromEntries(store)));
  } catch (error) {
    console.warn("Failed to persist demo store", error);
  }
}
function emit(table) {
...
function read(table) {
  hydrateFromStorage();
  return store.get(table) ?? EMPTY_ROWS;
}
...
function patch(table, id, partial) {
...
function subscribe(table, cb) {
  if (!listeners.has(table)) listeners.set(table, /* @__PURE__ */ new Set());
...
const ageRanges = ["18–25", "25–35", "35–45", "45–60", "60+"];
const events = [
  {
    id: "evt-1",
    ...
    zoneId: "kralingen",
    locationName: "Kralingen Court",
    pin: { x: 62, y: 48 },
    coords: "51.9231, 4.5321",
    startTime: "Today 18:00",
    startInMin: 30,
    organiser: "Rotterdam Sports",
    organiserVerified: true,
    status: "almost-full",
    isLive: false,
    minCapacity: 6,
    maxCapacity: 12,
    currentRegistration: 8,
    beginnerFriendly: true,
    skillLevel: "Beginner",
    ageRangeMin: 18,
    ageRangeMax: 99,
    accessibility: "Wheelchair accessible court",
    peopleComeAlone: true,
    welcomeHost: true,
    teamBased: true,
    teamSize: 3,
    spectatorsAllowed: true,
    bringFriendAllowed: true,
    distanceKm: 1.2,
  ...
  },
  {
    ...
    category: "Literature",
    subcategory: "Quiet reading",
    icon: "📚",
    description: "Bring a book. Read together in silence with tea. No talking expected.",
    zoneId: "kralingen",
    locationName: "Kralingen Library",
    pin: { x: 38, y: 36 },
    coords: "51.9242, 4.5289",
    startTime: "Sat 14:00",
    startInMin: 60 * 28,
    organiser: "Rotterdam Library",
    organiserVerified: true,
    status: "minimum-reached",
    isLive: false,
    minCapacity: 3,
    maxCapacity: 10,
    currentRegistration: 5,
    beginnerFriendly: true,
    skillLevel: "Any",
    ageRangeMin: 18,
    ageRangeMax: 99,
  ...
  },
  {
    ...
    category: "Cooking",
    subcategory: "Cultural food nights",
    icon: "🍲",
    description: "Cook and share a meal together. All skill levels welcome.",
    zoneId: "kralingen",
    locationName: "Buurthuis Kralingen",
    pin: { x: 54, y: 62 },
    coords: "51.9210, 4.5350",
    startTime: "Fri 19:00",
    startInMin: 60 * 50,
    organiser: "Buurthuis Kralingen",
    organiserVerified: true,
    status: "scheduled",
    isLive: false,
    minCapacity: 8,
    maxCapacity: 24,
    currentRegistration: 12,
    beginnerFriendly: true,
    skillLevel: "Any",
    ageRangeMin: 18,
    ageRangeMax: 99,
  ...
  },
  {
    ...
    zoneId: "kralingen",
    locationName: "Kralingse Plas — North entrance",
    pin: { x: 72, y: 30 },
    coords: "51.9281, 4.5301",
    startTime: "Today 19:30",
    startInMin: 120,
    organiser: "Wandelclub Rotterdam",
    organiserVerified: true,
    status: "scheduled",
    isLive: false,
    minCapacity: 3,
    maxCapacity: 30,
    currentRegistration: 9,
    beginnerFriendly: true,
    skillLevel: "Any",
    ageRangeMin: 18,
    ageRangeMax: 99,
  ...
  },
  {
    ...
    category: "Dance",
    subcategory: "Beginner dance",
    icon: "💃",
    description: "First-time friendly. Partners rotated. No experience needed.",
    zoneId: "centrum",
    locationName: "Dansstudio Centrum",
    pin: { x: 22, y: 64 },
    coords: "51.9201, 4.4810",
    startTime: "Sun 16:00",
    startInMin: 60 * 40,
    organiser: "Stichting Dans Rotterdam",
    organiserVerified: true,
    status: "scheduled",
    isLive: false,
    minCapacity: 6,
    maxCapacity: 20,
    currentRegistration: 4,
    beginnerFriendly: true,
    skillLevel: "Beginner",
    ageRangeMin: 18,
    ageRangeMax: 60,
  ...
  },
  {
    ...
    zoneId: "noord",
    locationName: "Café Buurt Noord",
    pin: { x: 48, y: 18 },
    coords: "51.9401, 4.4720",
    startTime: "Wed 18:30",
    startInMin: 60 * 70,
    organiser: "Welkom Rotterdam",
    organiserVerified: true,
    status: "scheduled",
    isLive: false,
    minCapacity: 4,
    maxCapacity: 20,
    currentRegistration: 7,
    beginnerFriendly: true,
    skillLevel: "Any",
    ageRangeMin: 18,
    ageRangeMax: 99,
...
const DEMO_USER_ID = "user_demo_resident";
const DEMO_USER_AUTH_ID = "auth_demo_resident";
const DEMO_ADMIN_ID = "admin_demo_rotterdam";
const DEMO_ADMIN_AUTH_ID = "auth_demo_admin";
function seedIfEmpty() {
  const seeded = read(TABLES.seeded);
  if (seeded.length) return;
  const demoResidentAuth = {
    id: DEMO_USER_AUTH_ID,
    email: "resident@demo.nl",
    password: "demo",
    role: "resident",
    created_at: nowIso()
  };
  const demoResident = {
    id: DEMO_USER_ID,
    auth_user_id: DEMO_USER_AUTH_ID,
    role: "resident",
    age_range: "18–25",
    gender_optional: "Prefer not to say",
    location_zone_id: "kralingen",
    radius_km: 4,
    onboarding_complete: true,
    selected_interests: ["sports", "Basketball", "literature", "Quiet reading", "cooking"],
    ...
    privacy_settings: { pauseRecs: false, hideFromMatching: false },
  ...
  const demoAdmin = {
    id: DEMO_ADMIN_ID,
    auth_user_id: DEMO_ADMIN_AUTH_ID,
    role: "municipality_admin",
    name: "Gemeente Rotterdam",
    municipality: "Rotterdam",
    created_at: nowIso()
  };
  write(TABLES.auth, [demoResidentAuth, demoOrgAuth, demoAdminAuth]);
  write(TABLES.users, [demoResident]);
  write(TABLES.organisers, [demoOrg]);
  write(TABLES.admins, [demoAdmin]);
  const events$1 = events.map((e) => {
  ...
  write(TABLES.aiProposals, [
    {
      ...
      proposal_reason: "AI detected high basketball interest in Kralingen tonight. 14 nearby users selected basketball. Weather is clear.",
      status: "proposed",
      created_at: nowIso()
    },
    {
      ...
      weather_note: "Indoor",
      proposal_reason: "7 nearby users selected karaoke this week. Venue KaraokeBar Centrum available Friday.",
      status: "proposed",
      created_at: nowIso()
    }
  ]);
  write(TABLES.seeded, [{ id: "seeded", at: nowIso() }]);
}
...
function createSupabaseClient() {
  const SUPABASE_URL = "https://xddrixfeapewytoetarr.supabase.co";
  const SUPABASE_PUBLISHABLE_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InhkZHJpeGZlYXBld3l0b2V0YXJyIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzkxMzIxNjcsImV4cCI6MjA5NDcwODE2N30.E2t8Cr79U8SGuTvbFY9y0j17xulgelniV8V2qendzJw";
...
function mapEventRow(r) {
  return {
    ...
    welcome_host_description: r.welcome_host_description ?? void 0,
    verified_organiser_required: r.verified_organiser_required,
    team_based: r.team_based,
    team_size: r.team_size ?? void 0,
...
async function hydrateAiProposals() {
  const { data } = await supabase.from("ai_event_proposals").select("*");
  write(TABLES.aiProposals, data ?? []);
}
async function hydrateUser(authId) {
  const { data } = await supabase.from("users").select("*").eq("auth_user_id", authId).maybeSingle();
  if (data) write(TABLES.users, [data]);
  return data;
}
async function hydrateOrganiser(authId) {
  const { data } = await supabase.from("organisers").select("*").eq("auth_user_id", authId).maybeSingle();
  if (data) {
    write(TABLES.organisers, [data]);
    const { data: vs } = await supabase.from("organiser_verification_steps").select("*").eq("organiser_id", data.id).maybeSingle();
    if (vs) write(TABLES.verification, [vs]);
  }
  return data;
}
async function hydrateAdmin(authId) {
  const { data } = await supabase.from("municipality_admins").select("*").eq("auth_user_id", authId).maybeSingle();
...
function localSessionForAccount(account) {
  if (account.role === "resident") {
    const profile2 = read(TABLES.users).find((u) => u.auth_user_id === account.id);
    if (!profile2) throw new Error("Local resident profile is missing.");
    return { auth_user_id: account.id, role: "resident", profile_id: profile2.id };
  }
  if (account.role === "organiser") {
    const profile2 = read(TABLES.organisers).find((o) => o.auth_user_id === account.id);
    if (!profile2) throw new Error("Local organiser profile is missing.");
    return { auth_user_id: account.id, role: "organiser", profile_id: profile2.id };
  }
  const profile = read(TABLES.admins).find((a) => a.auth_user_id === account.id);
  if (!profile) throw new Error("Local municipality admin profile is missing.");
  return { auth_user_id: account.id, role: "municipality_admin", profile_id: profile.id };
}
function signInLocal(email, password) {
  seedIfEmpty();
  const account = read(TABLES.auth).find(
    (a) => a.email.toLowerCase() === email.toLowerCase() && a.password === password
  );
  if (!account) return null;
  const session = localSessionForAccount(account);
  setSession(session);
  return session;
}
function createLocalResident(input) {
  seedIfEmpty();
  const exists = read(TABLES.auth).some(
    (a) => a.email.toLowerCase() === input.email.toLowerCase()
  );
  if (exists) throw new Error("A local demo account with this email already exists.");
  ...
  write(TABLES.auth, [...read(TABLES.auth), account]);
  write(TABLES.users, [...read(TABLES.users), user]);
  const session = { auth_user_id: authId, role: "resident", profile_id: userId };
  setSession(session);
  return session;
}
function createLocalOrganiser(input) {
  seedIfEmpty();
  const exists = read(TABLES.auth).some(
    (a) => a.email.toLowerCase() === input.email.toLowerCase()
  );
  ...
  const organiser = {
    ...
    verification_status: "verified",
    municipality_zone_id: input.zoneId,
    created_at: nowIso(),
    updated_at: nowIso()
  };
  write(TABLES.auth, [...read(TABLES.auth), account]);
  write(TABLES.organisers, [...read(TABLES.organisers), organiser]);
  const session = { auth_user_id: authId, role: "organiser", profile_id: organiserId };
  setSession(session);
  return session;
}
async function resolveSession(authUserId) {
  const u = await hydrateUser(authUserId);
  if (u) return { auth_user_id: authUserId, role: "resident", profile_id: u.id };
  const o = await hydrateOrganiser(authUserId);
  if (o) return { auth_user_id: authUserId, role: "organiser", profile_id: o.id };
...
const authService = {
  getSession() {
    return cachedSession ?? read(TABLES.session)[0] ?? null;
  },
  onChange(cb) {
    return subscribe(TABLES.session, cb);
  },
  async signUpResident(input) {
    return createLocalResident(input);
  },
  async signUpOrganiser(input) {
    return createLocalOrganiser(input);
  },
  async signUpAdmin(input) {
    {
      const local = signInLocal("admin@rotterdam.nl", "admin-demo-pass");
  ...
  async signInAdmin() {
    ...
    if (local) return local;
    throw new Error("Local demo admin account is missing.");
  },
...
const userService = {
  getById(id) {
    return read(TABLES.users).find((u) => u.id === id) ?? null;
  },
  getByAuthId(authId) {
    return read(TABLES.users).find((u) => u.auth_user_id === authId) ?? null;
  },
  async update(id, partial) {
    if (isLocalId(id)) {
      return patch(TABLES.users, id, { ...partial, updated_at: (/* @__PURE__ */ new Date()).toISOString() });
    }
    const { data, error } = await supabase.from("users").update({ ...partial, updated_at: (/* @__PURE__ */ new Date()).toISOString() }).eq("id", id).select().maybeSingle();
    if (error) throw new Error(error.message);
    if (data) write(TABLES.users, [data]);
    return data ?? null;
  },
  async completeOnboarding(id, data) {
  ...
  async refresh() {
    const s = authService.getSession();
    if (s && isLocalId(s.auth_user_id)) return;
...
const organiserService = {
  getById(id) {
    return read(TABLES.organisers).find((o) => o.id === id) ?? null;
  },
  getByAuthId(authId) {
    return read(TABLES.organisers).find((o) => o.auth_user_id === authId) ?? null;
  },
  async update(id, partial) {
    if (isLocalId(id)) {
      return patch(TABLES.organisers, id, {
        ...partial,
        updated_at: (/* @__PURE__ */ new Date()).toISOString()
      });
    }
    const { data, error } = await supabase.from("organisers").update({ ...partial, updated_at: (/* @__PURE__ */ new Date()).toISOString() }).eq("id", id).select().maybeSingle();
    if (error) throw new Error(error.message);
    if (data) write(TABLES.organisers, [data]);
    return data ?? null;
  },
...
const registrationService = {
  listByUser(userId) {
    return read(TABLES.registrations).filter((r) => r.user_id === userId);
  },
  listByEvent(eventId) {
    return read(TABLES.registrations).filter((r) => r.event_id === eventId);
  },
  forUserEvent(userId, eventId) {
    return read(TABLES.registrations).find(
      (r) => r.user_id === userId && r.event_id === eventId
    ) ?? null;
  },
  async setState(userId, eventId, state) {
    {
      const rows = read(TABLES.registrations);
      const idx = rows.findIndex((r) => r.user_id === userId && r.event_id === eventId);
      const next = idx >= 0 ? {
        ...rows[idx],
        participation_state: state,
        bring_friend: state === "Bring a friend",
        updated_at: nowIso()
      } : {
        id: uid("reg"),
        user_id: userId,
      ...
      write(TABLES.registrations, rows);
      return next;
    }
  },
  async cancel(userId, eventId) {
    {
      write(
        ...
        read(TABLES.registrations).filter(
          (r) => !(r.user_id === userId && r.event_id === eventId)
        )
      );
      return;
...
const checkInService = {
  listByEvent(eventId) {
    return read(TABLES.checkins).filter((c) => c.event_id === eventId);
  },
  countByEvent(eventId) {
    return checkInService.listByEvent(eventId).length;
  },
  hasUserCheckedIn(userId, eventId) {
    return read(TABLES.checkins).some(
      (c) => c.user_id === userId && c.event_id === eventId
    );
  },
  async create(userId, eventId, method = "im_here") {
    {
      const rows = read(TABLES.checkins);
      const existing = rows.find((c) => c.user_id === userId && c.event_id === eventId);
      ...
      write(TABLES.checkins, [...rows, checkIn]);
      const reg = read(TABLES.registrations).find(
        (r) => r.user_id === userId && r.event_id === eventId
      );
      if (reg) patch(TABLES.registrations, reg.id, { checked_in: true });
      return checkIn;
    }
  },
  onChange(cb) {
...
function AppProvider({ children }) {
  ...
  const setPrefs = (p) => {
    ...
    const patch2 = {};
    if (p.onboarded !== void 0) patch2.onboarding_complete = p.onboarded;
    if (p.ageRange !== void 0) patch2.age_range = p.ageRange;
    if (p.gender !== void 0) patch2.gender_optional = p.gender;
    if (p.zoneId !== void 0) patch2.location_zone_id = p.zoneId;
    if (p.radiusKm !== void 0) patch2.radius_km = p.radiusKm;
    if (p.interests !== void 0) patch2.selected_interests = p.interests;
    if (p.notifications !== void 0) patch2.notification_preferences = p.notifications;
    if (p.accessibility !== void 0) patch2.accessibility_preferences = p.accessibility;
    if (p.privacy !== void 0) patch2.privacy_settings = p.privacy;
    userService.update(user.id, patch2);
    if (p.organiserVerified && organiser) organiserService.markVerified(organiser.id);
  };
  const registrations = user ? registrationService.listByUser(user.id).map((r) => ({
    eventId: r.event_id,
    state: r.participation_state,
    checkedIn: r.checked_in
  })) : [];
  const setRegistration = (eventId, state) => {
    if (!user) return;
    try {
      registrationService.setState(user.id, eventId, state);
    } catch (e) {
      console.warn(e);
    }
  };
...
const Route$e = createRootRouteWithContext()({
  head: () => ({
    meta: [
      ...
      { title: "BuurtMatch — Nearby civic activities" },
      { name: "description", content: "Find nearby low-pressure social activities in your municipality. Backed by your Gemeente." },
      { name: "theme-color", content: "#F4FBF7" },
      { property: "og:title", content: "BuurtMatch" },
      { property: "og:description", content: "Find nearby low-pressure social activities, join without awkward messaging." },
...
function RootComponent() {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsx(QueryClientProvider, { client: queryClient, children: /* @__PURE__ */ jsxRuntimeExports.jsx(AppProvider, { children: /* @__PURE__ */ jsxRuntimeExports.jsx(Outlet, {}) }) });
}
const $$splitComponentImporter$d = () => import("./verify-BZd5OMZ3.js");
const Route$d = createFileRoute("/verify")({
  head: () => ({
    meta: [{
      title: "Get Verified — BuurtMatch"
    }, {
      name: "description",
      content: "Organiser verification flow."
    }]
  }),
  component: lazyRouteComponent($$splitComponentImporter$d, "component")
});
const $$splitComponentImporter$c = () => import("./suggested-B3Hod5Lb.js");
const Route$c = createFileRoute("/suggested")({
  head: () => ({
    meta: [{
      title: "Suggested events — BuurtMatch"
    }, {
      name: "description",
      content: "Meet people through things you already enjoy."
    }]
  }),
  component: lazyRouteComponent($$splitComponentImporter$c, "component")
});
const $$splitComponentImporter$b = () => import("./signup-Ba6ZuVsL.js");
const Route$b = createFileRoute("/signup")({
  head: () => ({
    meta: [{
      title: "Sign up — BuurtMatch"
    }]
  }),
  component: lazyRouteComponent($$splitComponentImporter$b, "component")
});
const $$splitComponentImporter$a = () => import("./settings-fSDR88wO.js");
const Route$a = createFileRoute("/settings")({
  head: () => ({
    meta: [{
      title: "Settings — BuurtMatch"
    }, {
      name: "description",
      content: "Manage preferences, accessibility, privacy and verification."
    }]
  }),
...
const $$splitComponentImporter$9 = () => import("./search-Bqqi7zrY.js");
const schema = object({
...
const Route$9 = createFileRoute("/search")({
  ...
  head: () => ({
    meta: [{
      title: "Search — BuurtMatch"
    }, {
      name: "description",
      content: "Search results split by radius."
    }]
  }),
  component: lazyRouteComponent($$splitComponentImporter$9, "component")
});
const $$splitComponentImporter$8 = () => import("./organiser-Cly-1v6R.js");
const Route$8 = createFileRoute("/organiser")({
  head: () => ({
    meta: [{
      title: "Organiser — BuurtMatch"
    }, {
      name: "description",
      content: "Create events and review AI proposals."
    }]
  }),
  component: lazyRouteComponent($$splitComponentImporter$8, "component")
});
const $$splitComponentImporter$7 = () => import("./onboarding-BsKJTyZ8.js");
const Route$7 = createFileRoute("/onboarding")({
  head: () => ({
    meta: [{
      title: "Welcome — BuurtMatch"
    }, {
      name: "description",
      content: "Lightweight onboarding for nearby civic activities."
    }]
  }),
  component: lazyRouteComponent($$splitComponentImporter$7, "component")
});
const $$splitComponentImporter$6 = () => import("./municipality-B6Ql_wnW.js");
const Route$6 = createFileRoute("/municipality")({
  head: () => ({
    meta: [{
      title: "Municipality Dashboard — BuurtMatch"
    }, {
      name: "description",
      content: "Aggregated, anonymous civic activity metrics."
    }]
  }),
  component: lazyRouteComponent($$splitComponentImporter$6, "component")
});
const $$splitComponentImporter$5 = () => import("./login-CI_22Nbo.js");
const Route$5 = createFileRoute("/login")({
  head: () => ({
    meta: [{
      title: "Sign in — BuurtMatch"
    }]
  }),
  component: lazyRouteComponent($$splitComponentImporter$5, "component")
});
const $$splitComponentImporter$4 = () => import("./interests-CzrXqrxx.js");
const Route$4 = createFileRoute("/interests")({
  head: () => ({
    meta: [{
      title: "My Interests — BuurtMatch"
    }, {
      name: "description",
      content: "Pick a few things you already enjoy."
    }]
  }),
  component: lazyRouteComponent($$splitComponentImporter$4, "component")
});
const $$splitComponentImporter$3 = () => import("./events-Dt0vgbhj.js");
const Route$3 = createFileRoute("/events")({
  head: () => ({
    meta: [{
      title: "Events — BuurtMatch"
    }, {
      name: "description",
      content: "Personal recommendations and nearby events."
    }]
  }),
  component: lazyRouteComponent($$splitComponentImporter$3, "component")
});
const $$splitComponentImporter$2 = () => import("./admin-match-BCLc1Tv2.js");
const Route$2 = createFileRoute("/admin-match")({
  head: () => ({
    meta: [{
      title: "Admin · Matching — BuurtMatch"
    }]
  }),
  component: lazyRouteComponent($$splitComponentImporter$2, "component")
});
const $$splitComponentImporter$1 = () => import("./index-BtYBfOne.js");
const Route$1 = createFileRoute("/")({
  head: () => ({
    meta: [{
      title: "Map — BuurtMatch"
    }, {
      name: "description",
      content: "Live map of nearby civic activities in your zone."
    }]
  }),
  component: lazyRouteComponent($$splitComponentImporter$1, "component")
});
const $$splitNotFoundComponentImporter = () => import("./event._id-DVxxpN3n.js");
const $$splitComponentImporter = () => import("./event._id-BsxgyKsO.js");
const Route2 = createFileRoute("/event/$id")({
  head: ({
    ...
    return {
      meta: [{
        title: e ? `${e.title} — BuurtMatch` : "Event — BuurtMatch"
      }, {
        name: "description",
        content: e?.description ?? "Civic event details."
      }]
    };
  },
...
const SuggestedRoute = Route$c.update({
  id: "/suggested",
  path: "/suggested",
  getParentRoute: () => Route$e
});
...
const OrganiserRoute = Route$8.update({
  id: "/organiser",
  path: "/organiser",
  getParentRoute: () => Route$e
});
...
const MunicipalityRoute = Route$6.update({
  id: "/municipality",
  path: "/municipality",
  getParentRoute: () => Route$e
});
...
const InterestsRoute = Route$4.update({
  id: "/interests",
  path: "/interests",
  getParentRoute: () => Route$e
});
...
const AdminMatchRoute = Route$2.update({
  id: "/admin-match",
  path: "/admin-match",
  getParentRoute: () => Route$e
});
...
export {
  ...
  interests as i,
  router as j,
  municipalityMetrics as m,
  nowIso as n,
  organiserService as o,
  read as r,
  subscribe as s,
  useApp as u,
  write as w,
